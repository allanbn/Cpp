...
double closest_pair(point *arrX, point *arrY, int i, int f) {
    int n = (f-i); // tamanho maximo a ser percorrido para preencher os arrays auxiliares de Y
    if ((n-1) <= 2) return closest_pair_bf(arrX, i, f);
    int m = (i+f) >> 1; // meio do array i e f que pode variar de 0 ao valor lido na entrada
    int im = n >> 1; // meio do intervalo entre i e f que determina a quantidade de elementos naquele intervalo
    point arrYL[im+1]; // array composto pelos Y's mais proximos a esqueda
    point arrYR[im+1]; // array composto pelos Y's mais proximos a direita
    int L = 0, R = 0;
    int j;
    for (j = 0; j < n; j++) {
    ...
    point arrY_aux[n+1]; // array auxiliar
    int nsize = 0;
    for (j = 0; j < n; j++) {
